(function() {
	var lists = (function() {
		...
		var getNormalizedEndPoint = function(container, offset) {
			...
			if (
				$_eyfb4cgljkmcwpc0.isListItemNode(container) &&
				$_eyfb4cgljkmcwpc0.isTextNode(node)
			) {
				var textNodeOffset =
		...
		var getSelectedSubLists = function(editor) {
			...
			if (isParentListSelected(parentList, selectedBlocks)) {
				...
				return global$5.grep(selectedBlocks, function(elm) {
					return (
						$_eyfb4cgljkmcwpc0.isListNode(elm) && parentList !== elm
					);
				});
			}
		};
		...
		var outdent = function(editor, li) {
			...
			if (
				$_eyfb4cgljkmcwpc0.isFirstChild(li) &&
				$_eyfb4cgljkmcwpc0.isLastChild(li)
			) {
				if (ulParent.nodeName === 'LI') {
					...
					ulParent.insertBefore(
						$_givla1gqjkmcwpcb.createNewTextBlock(editor, li),
						ul
					);
					DOM$4.remove(ul);
				}
			...
			if (ulParent.nodeName === 'LI') {
				ul = ulParent;
				newBlock = $_givla1gqjkmcwpcb.createNewTextBlock(
					editor,
					li,
					'LI'
				);
		...
		var outdentSelection = function(editor) {
			...
			if (listElements.length) {
				var bookmark = $_aredwjgjjkmcwpbt.createBookmark(
					editor.selection.getRng()
				);
				var i = void 0,
					y = void 0;
				var root = $_g5359xgnjkmcwpc5.getClosestListRootElm(
					editor,
				...
				editor.selection.setRng(
					$_aredwjgjjkmcwpbt.resolveBookmark(bookmark)
				);
				editor.nodeChanged();
				return true;
			}
		...
		var getSelectedTextBlocks = function(editor, rng, root) {
			...
			global$5.each(siblings, function(node) {
				...
				if (global$4.isBookmarkNode(node)) {
					if (
						$_eyfb4cgljkmcwpc0.isTextBlock(editor, nextSibling) ||
		...
		var applyList = function(editor, listName, detail) {
			...
			global$5.each(getSelectedTextBlocks(editor, rng, root), function(
				...
				if (
					sibling &&
					$_eyfb4cgljkmcwpc0.isListNode(sibling) &&
		...
		var removeList = function(editor) {
			...
			global$5.each(listItems, function(li) {
				...
				$_f0sxyjgmjkmcwpc3.normalizeLists(
					editor.dom,
					rootList.parentNode
				);
			});
		...
		var isValidLists = function(list1, list2) {
			return (
				list1 &&
				list2 &&
				$_eyfb4cgljkmcwpc0.isListNode(list1) &&
		...
		var toggleList = function(editor, listName, detail) {
			...
			var selectedSubLists = $_g5359xgnjkmcwpc5.getSelectedSubLists(
				editor
			);
		...
		var hasOnlyOneBlockChild = function(dom, elm) {
			...
			return (
				childNodes.length === 1 &&
				!$_eyfb4cgljkmcwpc0.isListNode(childNodes[0]) &&
		...
		var mergeLiElements = function(dom, fromElm, toElm) {
			...
			if (
				!$_eyfb4cgljkmcwpc0.isChildOfBody(dom, fromElm) ||
				!$_eyfb4cgljkmcwpc0.isChildOfBody(dom, toElm)
			) {
				return;
			}
		...
		var backspaceDeleteFromListToListCaret = function(editor, isForward) {
			...
			if (li) {
				...
				if (
					ul === editor.getBody() &&
					$_eyfb4cgljkmcwpc0.isEmpty(dom, ul)
				) {
					return true;
				}
		...
		var backspaceDeleteIntoListCaret = function(editor, isForward) {
			...
			if (block && dom.isEmpty(block)) {
				var rng = $_8n62cfgkjkmcwpbw.normalizeRange(
					editor.selection.getRng(true)
				);
				...
				if (otherLi_1) {
					editor.undoManager.transact(function() {
						...
						$_8bvl87gejkmcwpbi.mergeWithAdjacentLists(
							dom,
		...
		var backspaceDeleteRange = function(editor) {
			...
			if (
				startListParent ||
				$_g5359xgnjkmcwpc5.getSelectedListItems(editor).length > 0
			) {
				editor.undoManager.transact(function() {
		...
		var indent = function(li) {
			...
			if (
				sibling &&
				sibling.nodeName === 'LI' &&
				$_eyfb4cgljkmcwpc0.isListNode(sibling.lastChild)
			) {
			...
			return function(e) {
				...
				editor.on('NodeChange', function(e) {
					var tableCellIndex = findIndex(
						e.parents,
						$_eyfb4cgljkmcwpc0.isTableCellNode
					);
					var parents =
						tableCellIndex !== -1
		...
		var indentPostRender = function(editor) {
			return function(e) {
				...
				editor.on('nodechange', function() {
					var listItemBlocks = $_g5359xgnjkmcwpc5.getSelectedListItems(
						editor
					);
					var disable =
						listItemBlocks.length > 0 &&
						$_eyfb4cgljkmcwpc0.isFirstChild(listItemBlocks[0]);
