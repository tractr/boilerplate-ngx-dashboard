(function() {
	var spellchecker = (function() {
			...
			return function(method, text, doneCallback, errorCallback) {
				...
				global$3.send({
					url: new global$2(pluginUrl).toAbsolute(
						$_g9difkkbjkmcwpti.getRpcUrl(editor)
					),
					type: 'post',
		...
		var sendRpcCall = function(
			...
			var userSpellcheckCallback = $_g9difkkbjkmcwpti.getSpellcheckerCallback(
				editor
			);
		...
		var markErrors = function(
			...
			getTextMatcher(editor, textMatcherState)
				.find($_g9difkkbjkmcwpti.getSpellcheckerWordcharPattern(editor))
		...
		var get = function(
			...
			var getWordCharPattern = function() {
				return $_g9difkkbjkmcwpti.getSpellcheckerWordcharPattern(
					editor
				);
			};
			var markErrors = function(data) {
				$_d7tz2skcjkmcwptn.markErrors(
					editor,
					startedState,
					textMatcherState,
					lastSuggestionsState,
		...
		var getItems = function(editor) {
			return global$1.map(
				$_g9difkkbjkmcwpti.getLanguages(editor).split(','),
				function(langPair) {
		...
		var showSuggestions = function(
			...
			global$1.each(suggestions, function(suggestion) {
				items.push({
					...
					onclick: function() {
						...
						$_d7tz2skcjkmcwptn.checkIfFinished(
							editor,
							startedState,
							textMatcherState
						);
			...
			items.push({ text: '-' });
			var hasDictionarySupport = lastSuggestionsState.get()
				.hasDictionarySupport;
			if (hasDictionarySupport) {
				items.push({
					text: 'Add to Dictionary',
					onclick: function() {
						$_d7tz2skcjkmcwptn.addToDictionary(
							editor,
							pluginUrl,
							startedState,
							textMatcherState,
		...
		var setup = function(
			...
			editor.on('click', function(e) {
				...
				if (target.className === 'mce-spellchecker-word') {
					e.preventDefault();
					var spans = $_d7tz2skcjkmcwptn.findSpansByIndex(
						editor,
						$_d7tz2skcjkmcwptn.getElmIndex(target)
					);
					if (spans.length > 0) {
						var rng = editor.dom.createRng();
		...
		global.add('spellchecker', function(editor, pluginUrl) {
			if ($_b5sfxqk8jkmcwpsw.hasProPlugin(editor) === false) {
				...
				var currentLanguageState = Cell(
					$_g9difkkbjkmcwpti.getLanguage(editor)
				);
				var textMatcherState = Cell(null);
				var lastSuggestionsState = Cell(null);
				$_f9nm8ykjjkmcwpu4.register(
					editor,
					pluginUrl,
					startedState,
					textMatcherState,
