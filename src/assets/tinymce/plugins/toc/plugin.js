	...
	var toc = (function() {
		...
		var readHeaders = function(editor) {
			...
			var selector = generateSelector(
				$_7vx431sajkmcws8w.getTocDepth(editor)
			);
		...
		var generateTocHtml = function(editor) {
			...
			return (
				'<div class="' +
				editor.dom.encode($_7vx431sajkmcws8w.getTocClass(editor)) +
		...
		var generateTocContentHtml = function(editor) {
			...
			html += generateTitle(
				$_7vx431sajkmcws8w.getTocHeader(editor),
		...
		var setup = function(editor) {
			var $ = editor.$,
				tocClass = $_7vx431sajkmcws8w.getTocClass(editor);
		...
		var toggleState = function(editor) {
			return function(e) {
				...
				editor.on('LoadContent SetContent change', function() {
					ctrl.disabled(
						editor.readonly ||
							!$_1dj9nrs6jkmcws8q.hasHeaders(editor)
					);
				});
			};
		};
		var isToc = function(editor) {
			return function(elm) {
				return (
					...
					editor.dom.is(
						elm,
						'.' + $_7vx431sajkmcws8w.getTocClass(editor)
					) &&
