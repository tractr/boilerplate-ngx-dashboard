	...
	var link = (function() {
		...
		var link = function(editor, attachState) {
			return function(data) {
				editor.undoManager.transact(function() {
					...
					if (
						!$_7h5h1kfvjkmcwp8j.hasRelList(editor.settings) &&
						$_7h5h1kfvjkmcwp8j.allowUnsafeLinkTarget(
							editor.settings
						) === false
					) {
		...
		var showDialog = function(editor, linkList) {
			...
			var buildAnchorListControl = function(url) {
				var anchorList = [];
				global$4.each(editor.dom.select('a:not([href])'), function(
					anchor
				) {
			...
			onlyText = $_f96xu2g0jkmcwp91.isOnlyTextSelected(
				selection.getContent()
			);
			anchorElm = $_f96xu2g0jkmcwp91.getAnchorElement(editor);
			data.text = initialText = $_f96xu2g0jkmcwp91.getAnchorText(
				editor.selection,
				anchorElm
			);
			...
			if (anchorElm) {
				...
				$_7h5h1kfvjkmcwp8j.hasDefaultLinkTarget(editor.settings)
			) {
				data.target = $_7h5h1kfvjkmcwp8j.getDefaultLinkTarget(
					editor.settings
				);
			}
			...
			if ($_7h5h1kfvjkmcwp8j.shouldShowTargetList(editor.settings)) {
				if (
					$_7h5h1kfvjkmcwp8j.getTargetList(editor.settings) ===
					undefined
				) {
			...
			if ($_7h5h1kfvjkmcwp8j.hasRelList(editor.settings)) {
				relListCtrl = {
					...
					label: 'Rel',
					values: buildListItems(
						$_7h5h1kfvjkmcwp8j.getRelList(editor.settings),
						function(item) {
							if (
								$_7h5h1kfvjkmcwp8j.allowUnsafeLinkTarget(
									editor.settings
								...
								item.value = $_f96xu2g0jkmcwp91.toggleTargetRules(
									item.value,
									data.target === '_blank'
								);
							}
			...
			if ($_7h5h1kfvjkmcwp8j.hasLinkClassList(editor.settings)) {
				classListCtrl = {
					...
					label: 'Class',
					values: buildListItems(
						$_7h5h1kfvjkmcwp8j.getLinkClassList(editor.settings),
						function(item) {
							if (item.value) {
			...
			win = editor.windowManager.open({
				...
				onSubmit: function(e) {
					var assumeExternalTargets = $_7h5h1kfvjkmcwp8j.assumeExternalTargets(
						editor.settings
					);
					var insertLink = $_f96xu2g0jkmcwp91.link(
						editor,
						attachState
					);
		...
		var leftClickedOnAHref = function(editor) {
			return function(elm) {
				var sel, rng, node;
				if (
					$_7h5h1kfvjkmcwp8j.hasContextToolbar(editor.settings) &&
					!isContextMenuVisible(editor) &&
					$_f96xu2g0jkmcwp91.isLink(elm)
				) {
		...
		var toggleActiveState = function(editor) {
			return function() {
				...
				editor.on('nodechange', function(e) {
					self.active(
						!editor.readonly &&
							!!$_f96xu2g0jkmcwp91.getAnchorElement(
								editor,
								e.element
							)
					);
		...
		var setup = function(editor) {
			editor.addShortcut(
				'Meta+K',
				'',
				$_dlew49ftjkmcwp8f.openDialog(editor)
			);
		};
		...
		var setupContextToolbars = function(editor) {
			if (editor.addContextToolbar) {
				editor.addContextToolbar(
					$_dlew49ftjkmcwp8f.leftClickedOnAHref(editor),
