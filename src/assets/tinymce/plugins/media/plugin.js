	...
	var media = (function() {
		...
		var htmlToDataSax = function(prefixes, html) {
			...
			global$3({
				...
				start: function(name, attrs) {
					...
					if (name === 'script') {
						var videoScript = $_5qpz66h7jkmcwpf7.getVideoScriptMatch(
							prefixes,
							attrs.map.src
						);
						if (!videoScript) {
		...
		var dataToHtml = function(editor, dataIn) {
			...
			if (!data.source1) {
				global$2.extend(
					data,
					$_1f89fyh4jkmcwpf3.htmlToData(
						$_6qt1nth3jkmcwpf2.getScripts(editor),
						data.embed
					)
				);
				if (!data.source1) {
			...
			if (data.embed) {
				...
				var audioTemplateCallback = $_6qt1nth3jkmcwpf2.getAudioTemplateCallback(
					editor
				);
				var videoTemplateCallback = $_6qt1nth3jkmcwpf2.getVideoTemplateCallback(
					editor
				);
		...
		var showDialog = function(editor) {
			...
			var reserialise = function(update) {
				...
				win.find('#embed').value(
					$_6s992lhdjkmcwpfn.updateHtml(data.embed, data)
				);
			};
		...
		var retainAttributesAndInnerHtml = function(
			...
			if (innerHtml) {
				targetNode.attr(
					'data-mce-html',
					escape($_f1oicehmjkmcwpge.sanitize(editor, innerHtml))
				);
		...
		var placeHolderConverter = function(editor) {
			return function(nodes) {
				...
				while (i--) {
					...
					if (node.name === 'script') {
						videoScript = $_5qpz66h7jkmcwpf7.getVideoScriptMatch(
							$_6qt1nth3jkmcwpf2.getScripts(editor),
							node.attr('src')
						);
						if (!videoScript) {
					...
					if (
						node.name === 'iframe' &&
						$_6qt1nth3jkmcwpf2.hasLiveEmbeds(editor) &&
						global$1.ceFalse
					) {
		...
		var setup = function(editor) {
			editor.on('preInit', function() {
				...
				global$2.each('video audio iframe object'.split(' '), function(
					name
				) {
				...
				editor.parser.addNodeFilter(
					...
					$_f6hjinhljkmcwpga.placeHolderConverter(editor)
				);
					...
					function(nodes, name) {
						...
						while (i--) {
							...
							if (innerHtml) {
								...
								innerNode.raw = true;
								innerNode.value = $_f1oicehmjkmcwpge.sanitize(
									editor,
									unescape(innerHtml)
								);
				...
				if (target.getAttribute('data-mce-object')) {
					...
					if (html) {
						...
						target.setAttribute(
							'data-mce-html',
							escape(
								$_6s992lhdjkmcwpfn.updateHtml(html, {
									width: e.width,
									height: e.height
								})
							)
