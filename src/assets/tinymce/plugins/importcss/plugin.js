(function() {
	var importcss = (function() {
			...
			function append(styleSheet, imported) {
				...
				global$4.each(rules, function(cssRule) {
					if (cssRule.styleSheet) {
						...
						global$4.each(cssRule.selectorText.split(','), function(
							selector
						) {
		...
		var isExclusiveMode = function(editor, group) {
			return (
				group === null ||
				$_9eehcvfcjkmcwp6k.shouldImportExclusive(editor) !== false
			);
		};
		...
		var convertSelectorToFormat = function(
			...
			if (group && group.selector_converter) {
				...
				selectorConverter = $_9eehcvfcjkmcwp6k.getSelectorConverter(
					editor
				);
			} else {
		...
		var setup = function(editor) {
			editor.on('renderFormatsMenu', function(e) {
				...
				var selectorFilter = compileFilter(
						$_9eehcvfcjkmcwp6k.getSelectorFilter(editor)
					),
					ctrl = e.control;
				var groups = compileUserDefinedGroups(
					$_9eehcvfcjkmcwp6k.getCssGroups(editor)
				);
				...
				global$4.each(
					getSelectors(
						editor,
						e.doc || editor.getDoc(),
						compileFilter($_9eehcvfcjkmcwp6k.getFileFilter(editor))
					),
					function(selector) {
						if (selector.indexOf('.mce-') === -1) {
							if (!selectorFilter || selectorFilter(selector)) {
								...
								if (selectorGroups.length > 0) {
									global$4.each(selectorGroups, function(
										group
									) {
		...
		var get = function(editor) {
			var convertSelectorToFormat = function(selectorText) {
				return $_2llan7f7jkmcwp6d.defaultConvertSelectorToFormat(
					editor,
					selectorText
				);
			};
