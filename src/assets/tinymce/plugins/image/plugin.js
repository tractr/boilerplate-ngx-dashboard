	...
	var image = (function() {
		...
		var waitLoadImage = function(editor, data, imgElm) {
			...
			imgElm.onload = function() {
				if (
					!data.width &&
					!data.height &&
					$_gc1jdictjkmcwomd.hasDimensions(editor)
				) {
		...
		var getHspace = function(image) {
			if (
				...
				return $_9v7cmwcujkmcwomi.removePixelSuffix(
					image.style.marginLeft
				);
			} else {
				return '';
		...
		var setSize = function(name$$1, normalizeCss) {
			return function(image, name$$1, value) {
				if (image.style[name$$1]) {
					image.style[name$$1] = $_9v7cmwcujkmcwomi.addPixelSuffix(
						value
					);
				...
				if (
					...
					if (css['margin-top'] === css['margin-bottom']) {
						rootControl
							.find('#vspace')
							.value(
								$_9v7cmwcujkmcwomi.removePixelSuffix(
									css['margin-top']
								)
							);
					} else {
		...
		var onSrcChange = function(evt, editor) {
			...
			if (!meta.width && !meta.height) {
				...
				$_9v7cmwcujkmcwomi.getImageSize(
		...
		var getGeneralItems = function(editor, imageListCtrl) {
			...
			if ($_gc1jdictjkmcwomd.getClassList(editor)) {
				generalFormItems.push({
					...
					type: 'listbox',
					label: 'Class',
					values: $_9v7cmwcujkmcwomi.buildListItems(
						$_gc1jdictjkmcwomd.getClassList(editor),
						function(item) {
							if (item.value) {
		...
		var onFileInput = function(editor) {
			return function(evt) {
				...
				var uploader = Uploader({
					...
					credentials: $_gc1jdictjkmcwomd.getUploadCredentials(
						editor
					),
		...
		function Dialog(editor) {
			function showDialog(imageList) {
				...
				if (
					$_gc1jdictjkmcwomd.hasAdvTab(editor) ||
					$_gc1jdictjkmcwomd.hasUploadUrl(editor) ||
					$_gc1jdictjkmcwomd.hasUploadHandler(editor)
				) {
					var body = [
						$_bpqbuud9jkmcwoo5.makeTab(editor, imageListCtrl)
					];
					if ($_gc1jdictjkmcwomd.hasAdvTab(editor)) {
					...
					win = editor.windowManager.open({
						title: 'Insert/edit image',
						data: data,
						body: $_bpqbuud9jkmcwoo5.getGeneralItems(
							editor,
							imageListCtrl
						),
						onSubmit: curry(submitForm, editor)
