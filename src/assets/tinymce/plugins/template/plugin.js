(function() {
	var template = (function() {
		...
		var createTemplateList = function(editorSettings, callback) {
			return function() {
				var templateList = $_j4vdpr9jkmcws43.getTemplates(
					editorSettings
				);
		...
		var replaceVals = function(editor, e) {
			var dom = editor.dom,
				vl = $_j4vdpr9jkmcws43.getTemplateReplaceValues(editor);
		...
		var insertTemplate = function(editor, ui, html) {
			...
			html = replaceTemplateValues(
				editor,
				html,
				$_j4vdpr9jkmcws43.getTemplateReplaceValues(editor)
			);
			...
			global$1.each(dom.select('*', el), function(n) {
				if (
					...
					n.innerHTML = $_7qxvygrbjkmcws45.getDateTime(
						editor,
						$_j4vdpr9jkmcws43.getCdateFormat(editor)
					);
				}
				if (
					hasClass(
					...
					n.innerHTML = $_7qxvygrbjkmcws45.getDateTime(
						editor,
						$_j4vdpr9jkmcws43.getMdateFormat(editor)
					);
				}
				if (
					hasClass(
		...
		var register = function(editor) {
			editor.addCommand(
				'mceInsertTemplate',
				curry($_dv7lv7r6jkmcws40.insertTemplate, editor)
			);
		};
		...
		var setup = function(editor) {
			editor.on('PreProcess', function(o) {
				var dom = editor.dom,
					dateFormat = $_j4vdpr9jkmcws43.getMdateFormat(editor);
					...
					if (dom.hasClass(e, 'mceTmpl')) {
						global$1.each(dom.select('*', e), function(e) {
							if (
								...
								e.innerHTML = $_7qxvygrbjkmcws45.getDateTime(
									editor,
									dateFormat
								);
							}
		...
		var insertIframeHtml = function(editor, win, html) {
			...
			html = $_dv7lv7r6jkmcws40.replaceTemplateValues(
				editor,
				html,
				$_j4vdpr9jkmcws43.getPreviewReplaceValues(editor)
			);
		...
		var open = function(editor, templateList) {
			...
			win = editor.windowManager.open({
				...
				onsubmit: function() {
					$_dv7lv7r6jkmcws40.insertTemplate(
						editor,
						false,
						templateHtml
					);
		...
		var register$1 = function(editor) {
			editor.addButton('template', {
				title: 'Insert template',
				onclick: $_dv7lv7r6jkmcws40.createTemplateList(
					editor.settings,
					showDialog(editor)
				)
			});
