(function() {
	var charmap = (function() {
		...
		var insertChar = function(editor, chr) {
			var evtChr = $_cg0riu9qjkmcwo79.fireInsertCustomChar(editor, chr)
				.chr;
		...
		var open = function(editor) {
			...
			var charMapPanel = {
				type: 'container',
				html: $_f3h71t9wjkmcwo7r.getHtml(
					$_frviyp9rjkmcwo7a.getCharMap(editor)
				),
				onclick: function(e) {
					var target = e.target;
					if (/^(TD|DIV)$/.test(target.nodeName)) {
						...
						if (charDiv && charDiv.hasAttribute('data-chr')) {
							var charCodeString = charDiv.getAttribute(
								'data-chr'
							);
							var charCode = parseInt(charCodeString, 10);
							if (!isNaN(charCode)) {
								$_e3yubu9pjkmcwo78.insertChar(
									editor,
									String.fromCharCode(charCode)
								);
							}
