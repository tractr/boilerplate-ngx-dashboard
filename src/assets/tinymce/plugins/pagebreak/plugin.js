	...
	var pagebreak = (function() {
		...
		var setup = function(editor) {
			...
			var pageBreakSeparatorRegExp = new RegExp(
				separatorHtml.replace(/[\?\.\*\[\]\(\)\{\}\+\^\$\:]/g, function(
					a
				) {
					return '\\' + a;
				}),
					...
					while (i--) {
						...
						if (
							...
							if (
								...
								$_2s05hqi8jkmcwpjr.shouldSplitBlock(editor)
							) {
								parentNode.type = 3;
			...
			editor.addCommand('mcePageBreak', function() {
				if (editor.settings.pagebreak_split_block) {
					editor.insertContent(
						'<p>' + $_2m63szi6jkmcwpjp.getPlaceholderHtml() + '</p>'
					);
				} else {
					editor.insertContent(
						$_2m63szi6jkmcwpjp.getPlaceholderHtml()
					);
				}
			});
		};
		...
		var setup$1 = function(editor) {
			editor.on('ResolveName', function(e) {
				if (
					...
					editor.dom.hasClass(
						e.target,
						$_2m63szi6jkmcwpjp.getPageBreakClass()
					)
				) {
					e.name = 'pagebreak';
				}
