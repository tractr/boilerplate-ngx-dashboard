(function() {
	var textpattern = (function() {
		...
		var patternFromRng = function(patterns, rng, space) {
			...
			var endPattern = $_beghvss1jkmcws6v.findEndPattern(
				patterns,
				text,
				rng.startOffset,
				delta
			...
			var startPattern = $_beghvss1jkmcws6v.findPattern(
				patterns,
				patternRng.toString()
			);
			if (
		...
		var applyBlockFormat = function(editor, patterns) {
			...
			if (textBlockElm) {
				...
				if (firstTextNode) {
					pattern = $_beghvss1jkmcws6v.findPattern(
						patterns,
						firstTextNode.data
					);
					if (!pattern) {
		...
		function handleEnter(editor, patterns) {
			...
			wrappedTextNode = $_c3c0zdryjkmcws6m.applyInlineFormatEnter(
				editor,
				patterns
			);
			if (wrappedTextNode) {
		...
		function handleInlineKey(editor, patterns) {
			...
			wrappedTextNode = $_c3c0zdryjkmcws6m.applyInlineFormatSpace(
				editor,
				patterns
			);
			if (wrappedTextNode) {
		...
		var setup = function(editor, patternsState) {
			...
			editor.on('keyup', function(e) {
				if ($_g4f2z3rxjkmcws6k.checkKeyCode(keyCodes, e)) {
					$_g4f2z3rxjkmcws6k.handleInlineKey(
						editor,
						patternsState.get()
					);
				}
		...
		global.add('textpattern', function(editor) {
			var patternsState = Cell(
				$_1rurpvrtjkmcws6g.getPatterns(editor.settings)
			);
